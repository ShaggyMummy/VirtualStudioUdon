Shader "RedMage/Cancerspace Queue 3003" {
	Properties {
		...
		Pass {
			...
			fixed4 frag (v2f i) : SV_Target {
				...
				UNITY_LOOP for (int blurPass = 0; blurPass < _BlurSampling; ++blurPass) {
					...
					float2 sampleUV = grabUV + (blurNoiseRand.y * allAmp * _BlurRadius * float2(s, c)) / (_Garb3003_TexelSize.zw);
					...
					UNITY_UNROLL for (int j = 0; j < 3; ++j) {
						...
						if (_ScreenBoundaryHandling == BOUNDARYMODE_OVERLAY && (saturate(uv.x) != uv.x || saturate(uv.y) != uv.y)) {
							...
							UNITY_BRANCH if (_ScreenReprojection) {
								col[j] = tex2D(_Garb3003, uv * float2(VRFix, 1))[j];
							} else {
								col[j] = tex2D(_Garb3003, uv)[j];
							}
						}
					}
				...
				finalScreenColor = blend(tex2D(_Garb3003, i.projPos.xy / i.projPos.w).rgb, finalScreenColor, _OverallEffectMaskBlendMode, overallMask * overallMaskFalloff);
